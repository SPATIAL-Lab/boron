...
###########################################################################################  
LPEE.DSST <- rowMeans(jout$BUGSoutput$sims.list$tempC[,1:29]) - rowMeans(jout$BUGSoutput$sims.list$tempC[,431:435])
LPEE.Dpco2 <- rowMeans(jout$BUGSoutput$sims.list$pco2[,1:29]) - rowMeans(jout$BUGSoutput$sims.list$pco2[,431:435])
LPEE.pc.pco2 <- LPEE.Dpco2 / rowMeans(jout$BUGSoutput$sims.list$pco2[,1:29]) * 100
...
pPCIM.DSST <- rowMeans(jout$BUGSoutput$sims.list$tempC[,75:79]) - rowMeans(jout$BUGSoutput$sims.list$tempC[,109:112])
pPCIM.Dpco2 <- rowMeans(jout$BUGSoutput$sims.list$pco2[,75:79]) - rowMeans(jout$BUGSoutput$sims.list$pco2[,109:112])

PETM.DSST <- rowMeans(jout$BUGSoutput$sims.list$tempC[,181:209]) - rowMeans(jout$BUGSoutput$sims.list$tempC[,230:234])
PETM.DGMST <- rnorm(13500, mean = -5.6, sd=0.13)
PETM.Dpco2 <- rowMeans(jout$BUGSoutput$sims.list$pco2[,181:209]) - rowMeans(jout$BUGSoutput$sims.list$pco2[,230:234])
PETM.pc.pco2 <- PETM.Dpco2 / rowMeans(jout$BUGSoutput$sims.list$pco2[,181:209]) * 100
PETM.CS <- PETM.DSST / PETM.pc.pco2 * 100
PETM.CS <- subset(PETM.CS, PETM.CS<=10 & PETM.CS >= 0)
PETM.ESS <- PETM.DGMST / PETM.pc.pco2 * 100
PETM.ESS <- subset(PETM.ESCS, PETM.ESCS<=10 & PETM.ESCS >= 0)

ETM2.DSST <- rowMeans(jout$BUGSoutput$sims.list$tempC[,354:355]) - rowMeans(jout$BUGSoutput$sims.list$tempC[,382:386])
ETM2.Dpco2 <- rowMeans(jout$BUGSoutput$sims.list$pco2[,354:355]) - rowMeans(jout$BUGSoutput$sims.list$pco2[,382:386])
ETM2.pc.pco2 <- ETM2.Dpco2 / min(jout$BUGSoutput$sims.list$pco2[,354:355]) * 100
ETM2.CS <- ETM2.DSST / ETM2.pc.pco2 * 100
ETM2.CS <- subset(ETM2.CS, ETM2.CS<=10 & ETM2.CS >= 0)

plot(density(LPEE.CS), xlab = expression(paste("Regional Climate Sensitivity (",degree,"C / CO"[2], " doubling)")), col = rgb(0,0,0, alpha = 0), ylim=c(0,0.55), xlim=c(1,10), lwd=1.5)
lines(density(PETM.ESCS), col = "red", lty="dotted", lwd =1.5)
...
abline(v=median(PETM.ESCS), col="red", lty="dotted", lwd = 1.5)
...
print(c(median(PETM.ESS), quantile(PETM.ESS, 0.025), quantile(PETM.ESS, 0.975)))
...
print(c(mean(rowMeans(jout$BUGSoutput$sims.list$pco2[,354:355])), quantile(rowMeans(jout$BUGSoutput$sims.list$pco2[,354:355]), 0.025), quantile(rowMeans(jout$BUGSoutput$sims.list$pco2[,354:355]), 0.975)))
print(c(mean(rowMeans(jout$BUGSoutput$sims.list$pco2[,382:386])), quantile(rowMeans(jout$BUGSoutput$sims.list$pco2[,382:386]), 0.025), quantile(rowMeans(jout$BUGSoutput$sims.list$pco2[,382:386]), 0.975)))
...
# peak-PCIM cooling 
print(c(mean(-pPCIM.DSST), quantile(-pPCIM.DSST, 0.025), quantile(-pPCIM.DSST, 0.975)))
...
print(c(mean(rowMeans(jout$BUGSoutput$sims.list$pco2[,1:29])), quantile(rowMeans(jout$BUGSoutput$sims.list$pco2[,1:29]), 0.025), quantile(rowMeans(jout$BUGSoutput$sims.list$pco2[,1:29]), 0.975)))
...
# Latest Paleocene
LP.DSST <- rowMeans(jout$BUGSoutput$sims.list$tempC[,170:171]) - rowMeans(jout$BUGSoutput$sims.list$tempC[,182:185])
LP.Dpco2 <- rowMeans(jout$BUGSoutput$sims.list$pco2[,170:171]) - rowMeans(jout$BUGSoutput$sims.list$pco2[,182:185])
...
step.vector <- seq(1, length(ages.prox), by=1)
