...
############################################################################################    
  ...
  sal.m = 35  
  sal.p = 1/0.05^2    
  sal[1] ~ dnorm(sal.m, sal.p)  # prior initial distribution 
    # time series change terms:
  sal.phi = 1                   # temporal autocorrelation, phi, of a parameter
  sal.mc[1] = 1                 # initial mean change (relative no change = 1, additive no change = 0)
  sal.tau = 1/0.0005^2          # error precision for dt = 1kyr, tau, of a parameter 
  ...
  # Salinity (ppt)  
  sal.sig[i] ~ dnorm(sal.mc[i-1]*sal.phi^dt, sal.tau*((1-sal.tau^2)/(1-sal.tau^(2*dt))))I(0.9,1.1)
  sal[i] = sal[i-1] + sal.sig[i]
  
  # Temp in C
  tempC.sig[i] ~ dnorm(tempC.mc[i-1]*tempC.phi^dt, tempC.tau*((1-tempC.tau^2)/(1-tempC.tau^(2*dt))))
  tempC[i] = tempC[i-1] + tempC.sig[i]
  
  # [Ca] (mmol kg^-1)
  xca.sig[i] ~ dnorm(xca.mc[i-1]*xca.phi^dt, xca.tau*((1-xca.tau^2)/(1-xca.tau^(2*dt))))
  xca[i] = xca[i-1] + xca.sig[i]
  
  # [Mg] (mmol kg^-1)
  xmg.sig[i] ~ dnorm(xmg.mc[i-1]*xmg.phi^dt, xmg.tau*((1-xmg.tau^2)/(1-xmg.tau^(2*dt))))
  xmg[i] = xmg[i-1] + xmg.sig[i]
  
  # [SO4] (mmol kg^-1)
  xso4.sig[i] ~ dnorm(xso4.mc[i-1]*xso4.phi^dt, xso4.tau*((1-xso4.tau^2)/(1-xso4.tau^(2*dt))))
  xso4[i] = xso4[i-1] + xso4.sig[i]
  
  # d11B of seawater (per mille SRM-951) 
  d11Bsw.sig[i] ~ dnorm(d11Bsw.mc[i-1]*d11Bsw.phi^dt, d11Bsw.tau*((1-d11Bsw.tau^2)/(1-d11Bsw.tau^(2*dt))))
  d11Bsw[i] = d11Bsw[i-1] + d11Bsw.sig[i]
  
  # d18O of seawater (per mille SMOW) 
  d18Osw.sig[i] ~ dnorm(d18Osw.mc[i-1]*d18Osw.phi^dt, d18Osw.tau*((1-d18Osw.tau^2)/(1-d18Osw.tau^(2*dt))))I(0.9,1.1)
  d18Osw[i] = d18Osw[i-1] + d18Osw.sig[i]
  
  # pH
  pH.sig[i] ~ dnorm(pH.mc[i-1]*pH.phi^dt, pH.tau*((1-pH.tau^2)/(1-pH.tau^(2*dt))))
  pH[i] = pH[i-1] + pH.sig[i]
  
  # DIC (mol kg^-1) - make change in DIC temp dependent (C cycle model)?
  temp.dep[i] = (tempC[i]-tempC[i-1])*0.00000005
  dic.sig[i] ~ dnorm((dic.mc[i-1]+temp.dep[i])*dic.phi^dt, dic.tau*((1-dic.tau^2)/(1-dic.tau^(2*dt))))
