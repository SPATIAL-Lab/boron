...
############################################################################################    
  ...
  mgca_satdec ~ dnorm(0.054, 1/0.009^2)
  ...
  for (i in 1:length(ai.prox)){
    ...
    # Compute pCO2 from alk and pH 
    hyd[i] <- 10^(-(pH[ai.prox[i]]))
    BT[i] <- 4.3261e-4 * (sal[ai.prox[i]]/35)
    co2[i] <- (alk[ai.prox[i]] - ((KsB[i]*BT[i])/(KsB[i]+hyd[i])) - (Ksw[i]/hyd[i]) + hyd[i]) / ((Ks1[i]/hyd[i]) + 2*((Ks1[i]*Ks2[i])/(hyd[i]^2)))             
    fco2[i] <- co2[i] / K0[i]
    ...
    alpha.ccw[i] <- exp((A_Daeron*10^3*(1/temp[ai.prox[i]] - 1/297.7) + B_Daeron) / 10^3)
    epsilon.ccw[i] <- (alpha.ccw[i]-1)*10^3 
    d18Ofsmow[i] <- d18Osw.sc[i] + epsilon.ccw[i] 
    d18Ofpdb[i] <- 0.97001 * d18Ofsmow[i] - 29.99
    d18Ofdiag[i] <- d18Ofpdb[i] - (pH[ai.prox[i]] - 8)*d18O_pHcorr
    d18Of[i] <- d18Ofdiag[i]*(1-indexop) + d18Oseccal*indexop
    ...
    mgca_corr2[i] <- mgca_corr[i] / (1-(sal[ai.prox[i]]-35)*salcorrco)
    mgca_corr3[i] <- mgca_corr2[i] / (1-(pH[ai.prox[i]]-8.05)*mgca_pHcorr)
    mgcaf[i] <- mgca_corr3[i] + DDco3_corr[ai.prox[i]]*mgca_satdec
  }
...
############################################################################################    
  ...
  # Salinity (ppt)  
  sal[1] ~ dnorm(sal.m, sal.p)T(30,40)  
  sal.phi ~ dbeta(5,2)         
  sal.eps[1] = 0                 
  ...
  d18Osw[1] ~ dnorm(d18Osw.m, d18Osw.p)T(-1,-0.5)    
  ...
  pH.phi ~ dbeta(5,2)
  pH.eps[1] = 0 
  pH.tau ~ dgamma(2000, 1e-1) 
  
  # Alk surface (mmol kg^-1) 
  alk[1] ~ dnorm(alk.avg.pri[1], 1/alk.sd.pri[1]^2)
  
  # D[CO3=] of bottom water (umol kg^-1) difference from dissolution susceptibility limit of 21.3
  DDco3_corr[1] ~ dnorm(DDco3[1], 1/DDco3.sd[1]^2)
  ...
  for (i in 2:n.steps){
    ...
    d18Osw.eps[i] ~ dnorm(d18Osw.eps[i-1]*(d18Osw.phi^dt[i-1]), d18Osw.pc[i])T(-3, 2)
    ...
    alk[i] ~ dnorm(alk.avg.pri[i], 1/alk.sd.pri[i]^2)T(alk.min[i], alk.max[i])
    
    # D[CO3=] of bottom water (umol kg^-1) difference from dissolution susceptibility limit of 21.3
    DDco3_corr[i] ~ dnorm(DDco3[i], 1/DDco3.sd[i]^2)
  }
